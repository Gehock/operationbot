...
def bot(event_loop):
    ...
    # bot = Bot(command_prefix="!", intents=intents, loop=event_loop)
    # @bot.command()
    # async def ping(ctx):
    #     """Pong!"""
    #     await ctx.send("Pong!")
...
async def test_event_creation(bot, caplog):
    ...
    # logger.info(bool(msg.content("asd")))
    for message in dpytest.sent_queue._queue:
        logger.info(f"'{message.content}'")
    # msg._content == _Undef
    # logger.info(f"message: {vars(msg)}")
    # logger.info(f"message contains: {bool(msg.content('Pong!'))}")
